...
//    }
    ...
    public static boolean verifyElementTextContains(By by, String text) {
        ...
        boolean result = getTextElement(by).trim().toUpperCase().contains(text.trim().toUpperCase());
    ...
    public static boolean pressCopyPaste(String text) {
        smartWait();
    ...
    public static void doubleClickElement(By by) {
        ...
        act.doubleClick(waitForElementVisible(by));
        Log.info("Double click on element " + by.toString());

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Double clicked on the object " + by.toString());
        }
        addScreenshotToReport(Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
    }
